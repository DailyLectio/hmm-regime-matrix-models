...
10. **History CSV schema drift can corrupt post-SIM analysis.**

   The append-only history files currently have a 42-column header from an older schema, while newer appended rows contain 57 columns. The latest files are correct:

   - `NQ_Regimes_V3C_Latest.csv`: 57 columns, 57 values
   - `ES_Regimes_V3C_Latest.csv`: 57 columns, 57 values

   The history files are not safe for naive `Import-Csv` post-SIM analysis until they are archived/reset or rebuilt with the current 57-column header.
