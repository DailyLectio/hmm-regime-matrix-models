// CC BY-NC 4.0
// KalmanPulse_Fader_V2.cs — Adaptive Kalman Filter Mean-Reversion Fade Strategy
// ─────────────────────────────────────────────────────────────────────────────
...
namespace NinjaTrader.NinjaScript.Strategies
{
    public class KalmanPulse_Fader_V2 : Strategy
    {
        ...
        [Display(Name = "Max Total Contracts", GroupName = "2. Risk", Order = 3,
                 Description = "Hard cap across both legs. V2 default is 2 to prevent oversized risk-based quantities.")]
        public int MaxTotalContracts { get; set; } = 2;

        [NinjaScriptProperty, Range(0, 50)]
        [Display(Name = "Max Entry Signals Per Session (0=off)", GroupName = "3. Guards", Order = 3,
                 Description = "Counts a two-leg entry as one signal. Default 4 prevents repeat-fire churn days.")]
        public int MaxEntrySignalsPerSession { get; set; } = 4;

        [NinjaScriptProperty, Range(0, 50)]
        [Display(Name = "Exit Cooldown Bars", GroupName = "3. Guards", Order = 4,
                 Description = "Minimum bars after a flat/exit event before a new entry may fire.")]
        public int ExitCooldownBars { get; set; } = 3;

        [NinjaScriptProperty, Range(0, 3600)]
        [Display(Name = "Min Seconds Between Entry Signals", GroupName = "3. Guards", Order = 5,
                 Description = "Wall-clock throttle between entry signals. Default 60 seconds stops rapid re-entry loops.")]
        public int MinSecondsBetweenEntrySignals { get; set; } = 60;
        ...
        // Per-bar/session entry guards
        private int      lastEntryBar          = -1;
        private int      lastExitBar           = -1;
        private int      sessionEntrySignals   = 0;
        private DateTime lastEntrySignalTime   = DateTime.MinValue;
        private DateTime lastFlatExitTime      = DateTime.MinValue;
        ...
        private int CalcMaxContracts(double atrVal)
        {
            ...
            // V2 safety: keep the original risk-based sizing, but never allow it
            // to exceed the user-defined hard cap across both legs.
            int riskBasedQty = Math.Max(1, (int)(1500.0 / dollarRisk));
            return Math.Max(1, Math.Min(MaxTotalContracts, riskBasedQty));
        }

        private int ScaleByConfidence(int maxQty, int sizePct)
        {
            int scaled = Math.Max(1, (int)Math.Floor(maxQty * sizePct / 100.0));
            return Math.Max(1, Math.Min(MaxTotalContracts, scaled));
        }

        private bool IsEntryThrottleClear(DateTime now)
        {
            if (MaxEntrySignalsPerSession > 0 && sessionEntrySignals >= MaxEntrySignalsPerSession)
                return false;

            if (lastExitBar >= 0 && ExitCooldownBars > 0 && CurrentBar - lastExitBar < ExitCooldownBars)
                return false;

            if (MinSecondsBetweenEntrySignals > 0)
            {
                if (lastEntrySignalTime != DateTime.MinValue
                    && (now - lastEntrySignalTime).TotalSeconds < MinSecondsBetweenEntrySignals)
                    return false;

                if (lastFlatExitTime != DateTime.MinValue
                    && (now - lastFlatExitTime).TotalSeconds < MinSecondsBetweenEntrySignals)
                    return false;
            }
        ...
        private void MarkEntrySignal(DateTime now)
        {
            sessionEntrySignals++;
            lastEntrySignalTime = now;
        ...
        // =====================================================================
        private const string Stage1ModelVersion = "V2";
        private const string Stage1BotName = "KalmanPulse_Fader_V2";
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (Bars.IsFirstBarOfSession)
            {
                ...
                lastEntryBar       = -1;
                lastExitBar        = -1;
                sessionEntrySignals = 0;
                lastEntrySignalTime = DateTime.MinValue;
                lastFlatExitTime    = DateTime.MinValue;
                prevTickPrice      = 0;
                lastLeg1Target     = 0;
                lastLeg2Target     = 0;
            ...
            // Gate 1B: V2 anti-churn throttle: session cap + post-exit / wall-clock cooldown
            if (!IsEntryThrottleClear(Time[0])) { prevTickPrice = curPrice; return; }
