...
#
# Intended location:
#   C:\Users\Valued Customer\NT8_Regimes\V3C\RegimeMatrixSupervisor.py
#
# Inputs:
...
INSTRUMENTS = {
    "ES": {
        "macro": os.path.join(ACTIVE_DIR, "ES_Macro_Regimes.csv"),
        "micro": os.path.join(ACTIVE_DIR, "ES_Regimes.csv"),
        "history_output": os.path.join(OUTPUT_DIR, "ES_Regimes_V3C.csv"),
        "latest_output": os.path.join(OUTPUT_DIR, "ES_Regimes_V3C_Latest.csv"),
    },
    "NQ": {
        "macro": os.path.join(ACTIVE_DIR, "NQ_Macro_Regimes.csv"),
        "micro": os.path.join(ACTIVE_DIR, "NQ_Regimes.csv"),
...
def detect_stale_data(now_ts: pd.Timestamp, macro_ts: pd.Timestamp, micro_ts: Optional[pd.Timestamp]) -> Tuple[bool, str]:
    ...
    if micro_ts is not None:
        micro_age = (now_ts - micro_ts).total_seconds() / 60.0
        if micro_age > MAX_MICRO_AGE_MINUTES:
            return True, f"MICRO_STALE_{micro_age:.1f}MIN"
...
def process_instrument(instrument: str, paths: dict, now_ts: pd.Timestamp) -> Optional[dict]:
    df_macro = read_csv_safe(paths["macro"])
    df_micro = read_csv_safe(paths["micro"])
    ...
    # Avoid repeated duplicate output for the same macro checkpoint.
    if states[instrument].last_processed_macro_ts is not None:
        if macro_ts <= states[instrument].last_processed_macro_ts:
            return None
    ...
    while True:
        try:
            ...
            for inst, paths in INSTRUMENTS.items():
                if not os.path.exists(paths["macro"]) or not os.path.exists(paths["micro"]):
                    continue

                macro_mod = os.path.getmtime(paths["macro"])
                micro_mod = os.path.getmtime(paths["micro"])
