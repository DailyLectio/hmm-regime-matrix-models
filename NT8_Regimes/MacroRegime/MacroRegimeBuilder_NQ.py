...
class MacroRegimeBuilder:
    ...
    def _add_macro_labels(self, row: dict, cp: str) -> dict:
        ...
        # ==============================================================
        # V3 SURGICAL PATCH: BREAKING THE HIERARCHY (NQ SPECIALIST)
        # ==============================================================
        ib_ext = float(row.get("ib_extension_pct", 0.0))
        close_vs_vwap = float(row.get("close_vs_vwap_atr", 0.0))
        net_move = float(row.get("net_move_since_open_atr", 0.0))
        
        # 1. Lowered NQ Thresholds (More sensitive for tech volatility)
        if ib_ext >= 0.85:
            vol_state = "EXPANDING"
        elif ib_ext >= 0.45:
            vol_state = "NORMAL"
        ...
        ib_strong = ib_ext >= 0.85
        ib_expanding = ib_ext >= 0.60
        vwap_confirmed = abs(close_vs_vwap) > 0.40 and abs(net_move) > 0.60
...
if __name__ == "__main__":
    print("STARTING NQ MACRO REGIME BUILDER...")
    try:
        input_data_path = r"C:\Users\Valued Customer\NT8_Regimes\Exports\NQ_1min_export.txt"
        output_csv_path = r"C:\Users\Valued Customer\NT8_Regimes\Active\NQ_Macro_Regimes.csv"
